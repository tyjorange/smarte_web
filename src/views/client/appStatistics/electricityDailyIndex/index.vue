...
import { API_getCollector, API_findSignalsType, API_getSwitchByCollector, API_getSignalByDay } from './api.js'
...
export default {
  ...
  methods: {
    ...
    findSignalsType() {
      API_findSignalsType(getToken(), 'day').then(response => {
    ...
    handleClick(formData) {
      this.$refs[formData].validate((valid) => {
        if (!valid) {
          ...
          API_getSignalByDay(getToken(), this.ruleForm.switchObj.switchID, this.ruleForm.signalType.signalsTypeID, this.ruleForm.timeStr).then(response => {
            const a1 = []
            const a2 = []
    ...
    drawLine(a1, a2) {
      ...
      this.myChart.setOption({
        ...
        legend: {
          ...
          data: [this.ruleForm.signalType.typeName]
        },
        toolbox: {
          show: true,
          right: '2%',
        ...
        series: [
          {
            name: this.ruleForm.signalType.typeName,
            type: 'line',
            data: a2,
            // markPoint: {
            //   data: [
